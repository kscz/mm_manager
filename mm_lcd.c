...
int main(int argc, char *argv[]) {
    ...
    printf("Nortel Millennium Table Dump\n\n");

    lcd_table = (dlog_mt_npa_nxx_table_t *)calloc(1, sizeof(dlog_mt_lcd_table_t));
    ...
    if (size == LCD_TABLE_LEN - 1) {
        npa_char[1] = ((dlog_mt_lcd_table_t *)lcd_table)->called_npa[1];
    ...
    if (check_digit != 0xe) {
        printf("Invalid check digit %x, expected 0xe!\n", check_digit);
        free(lcd_table);
        return -1;
    }
    ...
    for (nxx = 200; nxx <= 999; nxx++) {
        ...
        case LCD_TABLE_LEN:
            ...
            flag_shift = 0;
            flag_mask = 0xFF;
//            printf(" O:%p/%p ", &((dlog_mt_lcd_table_t *)lcd_table)->lcd[nxx - 200], lcd_table);
            break;
        case COMPRESSED_LCD_TABLE_LEN:
            if (nxx % 2 == 0) {
                c = lcd_table->lcd[(nxx - 200) / 2];
                flag_mask  = 0xF0;
                flag_shift = 4;
            } else {
                flag_mask = 0x0F;
